...
import type { PageConfig, DeviceProfile, ActionHotZone, WidgetCtx, Action, PlacedWidget } from '../../shared/types';
...
function createWidgetCtx(
  ...
  return {
    ...
    entityValues,
    now,
    registerHotZone(bbox: { x: number; y: number; w: number; h: number }, action: Action, debug?: string) {
      const globalBbox = {
        x: bbox.x,
...
function PageView({ page, device, entityValues, now }: {
  page: PageConfig;
  device: DeviceProfile;
  entityValues: Record<string, unknown>;
  now: Date;
}) {
  const widgetCtx = createWidgetCtx(device, page, entityValues, now);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 18, color: '#606060' }}>{now.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</span>
      </div>
      {elements}
    </div>
  );
