...
export const BinarySensorWidget: WidgetSpec<BinarySensorConfig> = {
  ...
  render: (config, ctx) => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, border: '2px solid #a0a0a0', borderRadius: 4, backgroundColor: '#e8e8e8' }}>
        <span style={{ fontSize: 32, color: isOn ? '#101010' : '#808080' }}>{icon}</span>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <span style={{ fontSize: 16, color: '#606060' }}>{label}</span>
          <span style={{ fontSize: 20, fontWeight: 'bold', color: '#202020' }}>{isOn ? 'On' : 'Off'}</span>
        </div>
      </div>
    );
  },
